...
class ModelFolderObserver(
            ...
            activity.lifecycleScope.launch {
                val colors =
                    withContext(Dispatchers.IO) {
                        NoteApplicationService.get(activity).colors()
                            .toSet()
                    }
    ...
    internal fun label() {
        val baseNotes = baseModel.actionMode.selectedNotes.values.toList()
        activity.lifecycleScope.launch {
    ...
    private fun displaySelectLabelsDialog(labels: Array<String>, baseNotes: Collection<BaseNote>) {
        ...
        MaterialAlertDialogBuilder(activity)
            ...
            .setPositiveButton(R.string.save) { _, _ ->
                ...
                baseModel.applyBaseNoteLabels(
                    baseNotes.map { it.id }.toLongArray(),
                    checkedLabels,
                    uncheckedLabels.toSet(),
                )
            }
            .show()
    }
