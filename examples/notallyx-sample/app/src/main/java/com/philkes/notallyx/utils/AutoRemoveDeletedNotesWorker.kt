...
suspend fun ContextWrapper.removeOldDeletedNotes(): ListenableWorker.Result {
    ...
    return try {
        val removed = NoteApplicationService.get(this).emptyTrash(before)
        log(AutoRemoveDeletedNotesWorker.TAG, msg = "Auto removed $removed deleted notes after $days days")
