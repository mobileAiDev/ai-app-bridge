...
abstract class NotallyDatabase : RoomDatabase() {
    ...
    fun checkpoint() {
        openHelper.writableDatabase.query("PRAGMA wal_checkpoint(FULL)").use { cursor ->
            check(cursor.moveToFirst()) { "Database checkpoint returned no status" }
            val busy = cursor.getInt(0)
            val frames = cursor.getInt(1)
            val checkpointed = cursor.getInt(2)
            check(busy == 0 && (frames < 0 || checkpointed == frames)) {
                "Database checkpoint incomplete: busy=$busy, frames=$frames, checkpointed=$checkpointed"
            }
        }
    }
