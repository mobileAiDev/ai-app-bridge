...
class WidgetProvider : AppWidgetProvider() {
    ...
    private fun checkChanged(intent: Intent, context: Context) {
        ...
        val notes = NoteApplicationService.get(context)
        val pendingResult = goAsync()
        GlobalScope.launch {
            withContext(Dispatchers.IO) {
                try {
                    notes.checkItem(noteId, position, checked)
                } finally {
                    val app = context.applicationContext as NotallyXApplication
                    val preferences = NotallyXPreferences.getInstance(context)
                    updateWidgets(
    ...
    companion object {
        ...
        fun updateWidget(
            ...
            MainScope().launch {
                val color = withContext(Dispatchers.IO) { NoteApplicationService.get(context).color(noteId) }
                if (color == null) {
                    val app = context.applicationContext as Application
                    val preferences = NotallyXPreferences.getInstance(app)
