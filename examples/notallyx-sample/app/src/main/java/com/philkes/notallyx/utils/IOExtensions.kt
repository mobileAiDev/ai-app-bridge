...
fun ContextWrapper.deleteAttachments(
    ...
    if (attachments.isNotEmpty()) {
        ...
        attachments.forEachIndexed { index, attachment ->
            val (subfolder, name) = when (attachment) {
                is Audio -> SUBFOLDER_AUDIOS to attachment.name
                is FileAttachment -> (if (attachment.isImage) SUBFOLDER_IMAGES else SUBFOLDER_FILES) to attachment.localName
            }
            // A completed directory migration retains source copies for recovery. Delete both modes.
            (listOf(getPrivateAttachmentsRoot()) + externalMediaDirs.filterNotNull()).distinctBy { it.canonicalPath }.forEach { root ->
                val file = File(File(root, subfolder), name)
                if (file.exists()) check(file.delete()) { "Could not delete attachment ${file.name}" }
            }
