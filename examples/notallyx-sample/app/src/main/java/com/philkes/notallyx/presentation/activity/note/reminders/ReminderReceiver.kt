...
class ReminderReceiver : BroadcastReceiver() {
    ...
    private suspend fun deleteNote(context: Context, noteId: Long) {
        val store = notes(context)
        store.move(longArrayOf(noteId), Folder.DELETED)
    }
    ...
    private suspend fun unpinNote(context: Context, noteId: Long) {
        val store = notes(context)
        store.pinToStatus(longArrayOf(noteId), false)
    ...
    private suspend fun rescheduleAlarms(context: Context) {
        val store = notes(context)
        val now = Date()
        val noteReminders = store.reminders()
    ...
    private suspend fun setIsNotificationVisible(
        ...
        val store = notes(context)
        val note = store.note(noteId) ?: return
        ...
        if (index != -1) {
            if (currentReminders[index].isNotificationVisible != isNotificationVisible) {
                ...
                store.setReminders(noteId, currentReminders, notify = false)
            }
        }
    }
    ...
    private suspend fun restoreRemindersNotifications(context: Context) {
        val store = notes(context)
        val allNotes = store.activeNotes()
    ...
    private suspend fun restorePinnedNotifications(context: Context) {
        val store = notes(context)
        val allNotes = store.pinnedToStatusNotes()
        allNotes
    ...
    private fun notes(context: Context): NoteApplicationService = NoteApplicationService.get(context)
