...
open class PickNoteActivity : LockedActivity<ActivityPickNoteBinding>(), ItemListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val notesStore = NoteApplicationService.get(application)
        ...
        notesStore.observeFolder(com.philkes.notallyx.data.model.Folder.NOTES).observe(this) { currentNotes ->
            lifecycleScope.launch {
                val notes =
                    withContext(Dispatchers.IO) {
                        val raw =
                            currentNotes.filter { it.id != excludedNoteId }
