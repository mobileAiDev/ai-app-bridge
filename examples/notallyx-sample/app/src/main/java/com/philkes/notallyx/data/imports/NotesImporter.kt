...
class NotesImporter(private val app: Application, private val store: NoteApplicationService) {
    ...
    suspend fun import(
        ...
        try {
            ...
            val labels = notes.flatMap { it.labels }.distinct().sorted().mapIndexed { index, value -> Label(value, index) }
            ...
            importDataFolder?.let {
                importFiles(files, it, FileType.ANY, progress, totalFiles, counter)
                importFiles(images, it, FileType.IMAGE, progress, totalFiles, counter)
                importAudios(audios, it, progress, totalFiles, counter)
            }
            val result = store.importNotes(notes, labels, 0, true)
            progress?.postValue(ImportProgress(inProgress = false))
            return result
        } finally {
    ...
    private suspend fun importFiles(
        ...
        files.forEach { file ->
            ...
            val (fileAttachment, error) =
                if (fileType == FileType.IMAGE) app.importImage(uri, file.mimeType)
