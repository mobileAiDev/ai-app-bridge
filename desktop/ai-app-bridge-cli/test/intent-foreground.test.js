...
const bridge = require('../bin/ai-app-bridge');
...
const xml = `<hierarchy><node package="${picker}" class="android.widget.FrameLayout" bounds="[0,0][400,800]"><node package="${picker}" text="NotallyX Backup&#10;2026-09-07.zip" resource-id="${picker}:id/file_name" class="android.widget.TextView" enabled="true" clickable="true" bounds="[20,100][380,180]"/></node></hierarchy>`;
const native = { activity: `${home}.MainActivity`, root: { visible: true, effectiveVisible: true, enabled: true,
...
function harness(extra = {}) {
  ...
  const target = { serial: 'foreground-test', packageName: home, foregroundPackages: [picker] };
...
test('one supervised Intent observes native -> allowed system UIA -> native and attributes actions to the observed package', async () => {
  ...
  state = await h.worker.decide({ decisionId: 'open', basedOnRevision: state.revision, agentDecision: 'act', action: { action: 'tap', text: '导入', exact: true } });
  assert.equal(state.ok, true);
  ...
  assert.deepEqual(h.calls.filter(c => c[0] === 'tap').map(c => c[1]), [home, picker]);
  assert.equal(h.calls.filter(c => c[0] === 'tap')[1][4], undefined, 'system UI must use mechanical ADB input, not an app SDK action ID');
...
test('foreground changes after observation require reobserve and dispatch no tap', async () => {
  ...
  const result = await h.worker.decide({ decisionId: 'stale', basedOnRevision: state.revision, agentDecision: 'act', action: { action: 'tap', text: '导入' } });
...
test('a provider override cannot apply native coordinates to a UIA observation', async () => {
  ...
  const result = await h.worker.decide({ decisionId: 'wrong-provider', basedOnRevision: state.revision, agentDecision: 'act',
    action: { action: 'tap', provider: 'native', text: filename } });
  assert.equal(result.ok, false); assert.equal(result.error, 'observation_provider_mismatch');
  assert.equal(h.calls.filter(c => c[0] === 'tap').length, 0);
});

test('duplicate exact UIA text cannot silently pick the first file', async () => {
  const duplicate = xml.replace('</node></hierarchy>', `<node package="${picker}" text="NotallyX Backup&#10;2026-09-07.zip" enabled="true" bounds="[20,200][380,280]"/></node></hierarchy>`);
