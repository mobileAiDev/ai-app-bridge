...
const target={serial:'host-only-intent',packageName:'example.intent',port:18081,adb:'/test/adb'};
const tree={root:{text:'Save',visible:true,effectiveVisible:true,enabled:true,bounds:{left:10,top:20,right:30,bottom:40},children:[]}};
...
function decision(revision){return{decisionId:'d1',agentDecision:'act',basedOnRevision:revision,action:{action:'tap',text:'Save'}};}
...
test('Intent direct adapter throw is durably ambiguous and releases its target lease', async () => {
  resetIntentOperations();const evidence=store();
  ...
  assert.equal(lease.ok,true);lease.release();
});
...
test('Intent UIA mechanical tap does not masquerade as runtime-correlated dispatch', async () => {
  const calls=[];
  const adapter=createProductionIntentDeviceAdapter({ports:{createBridgeContext:options=>options,findUiaNodeByAny:()=>({left:0,right:20,top:0,bottom:20}),tap:async(...args)=>{calls.push(args);return{ok:true,transport:'adb'};}}});
  const receipt=await adapter.action({...target,actionId:'host-receipt-only',spec:{provider:'uia',action:'tap',text:'Save'},rawTree:'<node/>'});
  assert.equal(receipt.ok,true);
  assert.equal(calls[0].length,3);
  assert.equal(receipt.providerResult.transport,'adb');
});

test('Intent cancellation while prepared durability waits never dispatches a late side effect', async () => {
