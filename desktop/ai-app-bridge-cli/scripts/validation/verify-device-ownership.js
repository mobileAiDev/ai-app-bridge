...
// Real LocalSend and Android SDK, with an explicit Host HTTP fault proxy.
...
async function main({ out, serverPath, serial, packageName }) {
  ...
  const report = { ok: false, target, serverPath, scope: 'Real Android SDK and UI, controlled Host HTTP response loss and SIGKILL; not a blocked Dart peer', blocked: [] };
  const adb = (args) => execFileSync('adb', ['-s', serial, ...args], { encoding: 'utf8', timeout: 15000 });
  const businessHash = () => createHash('sha256').update(adb(['exec-out', 'run-as', packageName, 'cat', 'shared_prefs/FlutterSharedPreferences.xml'])).digest('hex');
  ...
  async function tree(c, predicate) {
    ...
    for (;;) {
      const value = check(await run(c, 'flutter-nodes', target));
      if (predicate(value)) return value;
      assert(Date.now() < deadline, 'Expected LocalSend page is absent'); await delay(150);
    }
  }
  ...
  try {
    ...
    proxy = http.createServer(async (req, res) => {
      try {
        ...
        if (req.url === '/v1/flutter/action') { accepted(item); return; }
        if (req.url === '/v1/flutter/cancel' && !allowCancel) { res.destroy(); return; }
    ...
    const initial = await tree(reader, t => t.nodes.some(n => n.text === '通过链接接收'));
    const boundary = check(await run(reader, 'events', { ...target, view: 'decision-window', limit: 100 }));
    const actionId = `ownership-${Date.now()}`;
    const pending = run(owner, 'tap-flutter', { ...target, adb: faultAdb, selector: { text: '发送' }, requestId: actionId,
      timeoutMs: 30000, feedback: 'off' }).then(value => ({ value }), error => ({ error: error.message }));
    const effect = await Promise.race([effectReceived, delay(12000).then(() => { throw new Error('No actual SDK completion reached the fault proxy'); })]);
    ...
    report.action = { actionId, runtimeEpoch: initial.runtimeEpoch, actualSdkReceipt: effect.response, hostResponseDelivered: false };
    ...
    const intent = check(await run(recovery, 'intent', { operation: 'start', provider: 'flutter', goal: 'Ownership denial probe; must not dispatch.', target: { platform: 'android', ...target } }));
    const decided = await run(recovery, 'intent', { operation: 'decide', operationId: intent.operationId, decision: {
      decisionId: 'blocked', basedOnRevision: intent.revision, agentDecision: 'act', action: { provider: 'flutter', action: 'tap', selector: { text: '发送' } } } });
    ...
    assert(observed.items.some(e => e.actionId === actionId && e.name === 'flutter.action.settled'));
    report.phoneEvidence = { events: observed.items.filter(e => e.actionId === actionId), refs: observed.refs };
    await tree(recovery, t => t.nodes.some(n => n.text === '选择'));
    check(await run(recovery, 'screenshot', { ...target, outFile: path.join(out, 'send-after-owner-crash.png'), feedback: 'off' }));
    allowCancel = true;
    ...
    const current = await tree(recovery, t => t.nodes.some(n => n.text === '选择'));
    const receive = current.nodes.filter(n => n.text === '接收' && n.widgetType === 'NavigationDestination');
    assert.equal(receive.length, 1);
    check(await run(recovery, 'tap-flutter', { ...target, selector: { nodeId: receive[0].id }, feedback: 'off' }));
    await tree(recovery, t => t.nodes.some(n => n.text === '通过链接接收'));
    ...
    if (!report.ok && recovery) {
      try {
        ...
        if (report.cleanupRecovery.ok) {
          const current = check(await run(recovery, 'flutter-nodes', target));
          const receive = current.nodes.filter(n => n.text === '接收' && n.widgetType === 'NavigationDestination');
          if (receive.length === 1) report.cleanupRestore = await run(recovery, 'tap-flutter', { ...target, selector: { nodeId: receive[0].id }, feedback: 'off' });
        }
