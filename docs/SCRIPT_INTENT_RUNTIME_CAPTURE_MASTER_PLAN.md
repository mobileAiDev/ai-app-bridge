...
0.3.0 正式发布与默认入口切换已完成。本轮按用户要求升级 Reader 并验证实际 Intent → Script 流程，同时将 skill、MCP 说明改为精简且按需加载，各端统一准备 0.3.1。当前收尾是该补丁的打包、公开坐标与 Reader 最新依赖检查，不扩大 App 或验收矩阵；详见[0.3.1 交接](RELEASE_HANDOFF_2026-09-12.md#031按需说明与-reader-接入)。

以下为 0.3.0 工程封版范围，保留原证据边界。

用户最新要求停止主开发反复扩大业务测试，先完成整体工程收尾，现已授权发布并将默认入口统一到正式版 `0.3.0`。当前优先级以[收尾审查](ENGINEERING_CLOSEOUT_REVIEW_2026-09-12.md)和[唯一执行清单](BRIDGE_NEXT_GATES_2026-09-08.md#2026-09-12-样本收敛与当前顺序)为准。

共享 Runtime、CLI/MCP、Intent/Script、手机真实存查主架构已经成立。主开发已关闭完整 Script result 持久读取、旧 Flutter H5 目标绑定、动作预算合同、公开依赖/接入与发行文档等具体缺口，形成 `0.3.0` 发行版本，定向代码检查、干净 npm 包、根 SPM 和 Android 发行产物均已核验；见[发布交接](RELEASE_HANDOFF_2026-09-12.md)。当前执行正式发布与默认入口切换，不再重开基础开发轮次。原四 App、现有 iOS 原生/H5/Flutter、跨设备与最终生产范围保持，剩余业务验收移交固定清单，不再不断扩样本或重跑未改变的矩阵。
