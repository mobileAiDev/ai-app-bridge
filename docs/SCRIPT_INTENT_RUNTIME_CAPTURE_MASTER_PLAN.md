...
0.3.0 正式发布与默认入口切换已完成。本轮按用户要求升级 Reader 并验证实际 Intent → Script 流程，同时将 skill、MCP 说明改为精简且按需加载，各端统一准备 0.3.2。当前收尾是该补丁的打包、公开坐标与 Reader 最新依赖检查，不扩大 App 或验收矩阵；详见[0.3.2 交接](RELEASE_HANDOFF_2026-09-12.md#032保留失败记录后重建)。
