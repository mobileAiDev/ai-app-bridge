...
Install the debug APK, then use Bridge 0.3.8 `flutter-executor` operations `open`, `observe`, `act`, `receipt`, `close`. Python and JavaScript Script both call them through `ctx.call` with `app.test` permission. Workflows are not recompiled into the application for each run.
