...
internal object AndroidH5Renderer {
    val source = """
        (function(request) {
          const key = '__aabAndroidH5TargetV1';
          ...
          const all=controls();
          if (all.length>5000) return fail('android_h5_target_scan_truncated');
          ...
          if (reachability.status==='outside-viewport') return fail('android_h5_target_outside_viewport');
          if (reachability.status!=='ready') return fail('android_h5_target_obscured');
          ...
          if (request.action==='click') {
            if (typeof element.click!=='function') return fail('android_h5_target_not_clickable');
          ...
          if (request.action==='input') {
            ...
            if (!editable(element)) return fail('android_h5_target_not_editable');
            const before=valueControl?element.value:element.innerText;
            element.focus();
            if (!current()||!editable(element)||(valueControl?element.value:element.innerText)!==before) return fail('android_h5_target_changed',true);
            ...
            if (!document.documentElement.contains(element)) return fail('android_h5_target_changed',true);
